...
const getTemplateId = (key: string, value: string | undefined, defaultValue: string) => {
    ...
    // 프로덕션 환경(VITE_LANDING_ONLY가 false이고 localhost가 아닌 경우)에서 템플릿 ID가 없거나 placeholder인 경우 경고
    if (typeof window !== 'undefined' &&
        !window.location.hostname.includes('localhost') &&
        (!id || id.startsWith('template_') || id.includes('placeholder'))) {
        console.warn(`⚠️ [Solapi] Potential Invalid Template ID for ${key}: ${id}. Check templates.json or Vercel env vars.`);
    }
    return id;
};
...
export const TEMPLATES = {
    ...
    PUBLIC_ROOM_FIRST_MATCH: getTemplateId('PUBLIC_ROOM_FIRST_MATCH', templateIds.PUBLIC_ROOM_FIRST_MATCH, 'template_04_public'),
    ...
    NO_REFUND_NOTICE: getTemplateId('NO_REFUND_NOTICE', templateIds.NO_REFUND_NOTICE, 'template_19'),
    MATCH_REMINDER: getTemplateId('MATCH_REMINDER', templateIds.MATCH_REMINDER, 'template_20'),
    STUDENT_ID_REJECTED: getTemplateId('STUDENT_ID_REJECTED', templateIds.STUDENT_ID_REJECTED, 'template_21'),

    // 22: 선택 시간 안내
    DECISION_TIME: getTemplateId('DECISION_TIME', templateIds.DECISION_TIME, 'template_22'),
};
